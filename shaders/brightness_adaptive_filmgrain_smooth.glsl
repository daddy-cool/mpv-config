...
#define INTENSITY grain_intensity
#define TAPS 5
...
void hook() {
    ...
    // adjust grain intensity based on brightness, but only if the image is not completely black
    if (brightness > 0.00001) {
        if (brightness > 1.0) {
            brightness = 1.0;
        }
        color.rgb += vec3(INTENSITY * vsum * brightness);
    }
